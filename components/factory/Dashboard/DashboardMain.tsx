...
export default function DashboardMain({
  ...
  const refreshWithIngest = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Ingest error:", txt);
        throw new Error("Daily ingest failed");
      }
  ...
  /* ================= INITIAL LOAD (NO INGEST) ================= */
  useEffect(() => {
    if (!factoryId) return;
    loadOnly();
  }, [factoryId, range, loadOnly]);

  /* ================= REGISTER REFRESH ONCE ================= */
  useEffect(() => {
  ...
  const emissionLoadKg =
  data?.emissionEstimate?.dailyKgEstimate?? null;
  ...
  return (
    <div className={`space-y-6 ${refreshing ? "opacity-60" : ""}`}>
      <MainDashboardStats
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="xl:col-span-2 rounded-2xl border border-white/10 bg-[#0b1220] p-5">
          <div className="mb-3 flex justify-between">
            <div>
              <h3 className="font-semibold">Environmental Risk Trend</h3>
      ...
      <div className="grid grid-cols-2 gap-6">
        <SingleGasTrendChart data={history} gas="no2" label="NO₂" color="#38bdf8" unit="ppm" />
        <SingleGasTrendChart data={history} gas="so2" label="SO₂" color="#22c55e" unit="ppm" />
        <SingleGasTrendChart data={history} gas="co" label="CO" color="#facc15" unit="ppm" />
        <SingleGasTrendChart data={history} gas="pm25" label="PM2.5" color="#ef4444" unit="µg/m³" />
      </div>
    </div>
  );
}
